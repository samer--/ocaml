...
let find_min x =
 let rec loop = function
  ...
  | Node (l, _, _, _, _)     -> loop l
  | Empty -> raise Not_found
 in loop x.map


let rec delete_find_min' = function
  | Node (Empty, k, v, r, _) -> ((k,v),r)
  | Node (l, k, v, r, _) -> 
      let (b,l') = delete_find_min' l in
      (b,bal l' k v r)
  | Empty -> invalid_arg "PMap.delete_find_min"

let delete_find_min x = 
  let (b,t) = delete_find_min' x.map
  in (b, {cmp = x.cmp; map = t})

let rec delete_find_max' = function
  | Node (l, k, v, Empty, _) -> ((k,v),l)
  | Node (l, k, v, r, _) -> 
      let (b,r') = delete_find_max' r in
      (b,bal l k v r')
  | Empty -> invalid_arg "PMap.delete_find_max"

let delete_find_max x = 
  let (b,t) = delete_find_max' x.map
  in (b, {cmp = x.cmp; map = t})
...
let merge t1 t2 =
  ...
  | _, Empty -> t1
  | _ ->
      let ((k,v),t2') = delete_find_min' t2 in
      bal t1 k v t2'
...
let update_with f x { cmp = cmp; map = map } =
  let rec loop = function
    | Node (l, k, v, r, h) ->
        let c = cmp x k in
        if c = 0 then 
          let (y,v')=f v in (y,Node (l, x, v', r, h))
        else if c < 0 then
          let (y,nl) = loop l in
          (y,bal nl k v r)
        else
