...
val update_with : ('v -> 'a *'v) -> 'k ->  ('k, 'v) t -> 'a * ('k, 'v) t
(**
Added by Samer Abdallah for more efficien updating. 
[update_with f key map] will update the entry specified by [key] by
calling [f old_val], which must return a pair [(result,new_val)].
[new_val] is installed into the map and [result] is returned along
with the new map.  *)
