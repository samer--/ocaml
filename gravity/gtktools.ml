...
let with_system (system: 's system) action = 
  ...
  w#show ();
  let result = action area sref draw_cr in
  w#destroy ();
  print_endline "Leaving..";
  result

let animate_with_timeouts tau system = 
  with_system system (fun area _ _ ->
    let animate () = 
      let _ = GtkBase.Widget.queue_draw area#as_widget in true
    in ignore (Glib.Timeout.add (int_of_float (1000.0 *. tau)) animate);
    GMain.main ())

let animate_with_loop stop tau system = 
  with_system system (fun area sref draw_cr ->
    let rec check_pending t = 
      if not (Glib.Main.pending ()) then update t
      else if Glib.Main.iteration false && not (stop sref) then check_pending t 
      else () 
    and update t =
